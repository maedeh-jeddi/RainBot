...
`worlds/hospital_lab.sdf` is a second world, built for a **blood sample
transport** run: collect a rack of samples from a lab bench, carry it the length
of a corridor, and set it down in a ward at the far end. It is a building, not
an open floor:
...
Bring it up with the two environment variables `gazebo.launch.py` already
reads - the world origin is inside the lab's north wall, so the spawn point has
...
It has a saved map, `maps/hospital_lab.yaml`. To localize and navigate on it:

```bash
WORLD=hospital_lab.sdf SPAWN_X=-6.5 SPAWN_Y=0.0 \
    ros2 launch pickplace_arm_bringup localization.launch.py \
    map:=$(ros2 pkg prefix pickplace_arm_bringup)/share/pickplace_arm_bringup/maps/hospital_lab.yaml
```

AMCL needs no reseeding: mapping started from the spawn pose, so the map frame's
origin is the spawn point and `amcl_tugbot.yaml`'s (0, 0, 0) initial pose is
already correct. Measured against Gazebo ground truth with the robot at rest,
AMCL sits **0.03 m** off.

The mission launch still loads `maps/tugbot_warehouse.yaml`, because the sample
transport mission itself does not exist yet - only the world, the props and the
map do.

### Rebuilding the map

The map is reproducible rather than depending on how somebody happened to drive:

```bash
WORLD=hospital_lab.sdf SPAWN_X=-6.5 SPAWN_Y=0.0 \
    ros2 launch pickplace_arm_bringup mapping.launch.py
ros2 run pickplace_arm_bringup hospital_map_drive    # 2nd terminal
ros2 run nav2_map_server map_saver_cli -f src/pickplace_arm_bringup/maps/hospital_lab
```

`hospital_map_drive` replaces a human on `teleop_key`: it drives a fixed 33-point
tour at 0.30 m/s, steering by the SLAM-corrected `map -> base_link` pose rather
than raw odometry, with a LIDAR watchdog that abandons a waypoint instead of
driving into anything.

**After editing that tour, re-run `ros2 run pickplace_arm_bringup
hospital_route_check`.** It measures every waypoint and every segment between
waypoints against every prop footprint in the world. This is not ceremony: the
first tour was checked only at its waypoints, and the straight line between two
perfectly clear waypoints ran directly through a standing figure. The checker
requires 0.62 m at waypoints (the Husky's 0.598 m circumscribed radius, since it
pivots there) and 0.45 m along segments (its 0.335 m half width plus margin).

The pocket of the ward directly behind the delivery bench stays unmapped. That is
deliberate - the gaps either side of the bench are too narrow for the Husky, so
the robot cannot drive there and the mission never needs to.

### Layout and clearances

The corridor is 3.0 m wide and everything in it is placed against a wall, which
leaves at least 1.7 m of clear width past the tightest obstacle. The Husky is
0.67 m wide, so Nav2 always has room to *pass* something rather than stop dead
in front of it - the point of the world is a run that does not have to halt.

Prop placement is measured, not eyeballed: each Fuel model's footprint was read
off its own mesh (min/max of the OBJ vertices times the model's `<scale>`) and
the poses were chosen so no two footprints overlap and nothing encroaches on the
driving lane. Every one of these models puts its origin on the floor, so they
all spawn at `z = 0`.

### People are models, not actors

The pedestrians are Fuel `Walking person` **models** - mesh collision, 80 kg
inertial - not SDF `<actor>`s. That distinction decides whether the world is
usable at all: an actor in gz-sim carries no collision geometry, so it is
invisible to contact physics and cannot be an obstacle Nav2 must genuinely
avoid. The seated and standing staff (`Nurse`, `Scrubs`, visitors, patients) are
static mesh models and are real obstacles too.

They are pinned `<static>` for now, so they stand still. Dropping that tag and
driving them with a velocity or trajectory system is what turns them into moving
pedestrians; the collision geometry is already in place for when they do.

### The two new props

| Model | What it is |
| --- | --- |
| `lab_bench` | Lab counter with a low transfer shelf. Fixed building furniture, so the world pulls it in by `model://lab_bench` rather than spawning it from a launch file. |
| `sample_rack` | Six capped blood tubes in a carrier with a red grip block. Mission payload, spawned like the coloured boxes are. |

The shelf height is the load-bearing detail. The front RGB-D camera is bolted to
the Husky's front bumper 0.223 m above the floor, so anything standing on a
realistic 0.90 m counter top is far above its horizon and cannot be detected on
approach. The shelf puts the payload at 0.30 m - the same height as the table the
colour-sorting mission already picks from - so the existing detection and
approach geometry carry over unchanged while the bench still reads as a real lab
counter.

The rack's grip block is a 0.06 m cube for the same reason: it is exactly what
the coloured boxes present to the jaws, so `grab_below`'s finger-gap grasp check
still holds, and being a cube rather than a flat tab it grasps from any approach
yaw. It is saturated red so the existing HSV red detector finds it with no new
perception code.

The rack gets the same `DetachableJoint` rigid grasp as the boxes, and the same
caveat applies with more force: **the joint is created the instant the rack
spawns**, so a mission must publish `/sample_rack/detach` before driving off or
the robot will drag the rack off the bench with it.
...
- [x] Hospital world: lab, corridor and ward, with staff and pedestrians as
      real collision obstacles
- [x] Blood sample props: lab transfer bench and a graspable sample rack
- [x] Saved map for the hospital world, with a reproducible mapping drive
- [ ] Nav2 footprint fix: `robot_radius` is still 0.25 m, sized for the
      pre-Husky chassis (the A200 needs ~0.60 m)
- [ ] Blood sample transport mission (lab to ward)
- [ ] Smooth transport: acceleration and jerk limits so the payload is not shaken
- [ ] Predictive pedestrian avoidance (steer around rather than brake)
- [ ] Moving pedestrians (drive the `Walking person` models along paths)
