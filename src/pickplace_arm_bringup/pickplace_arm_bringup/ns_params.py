...
# Keys whose VALUE names a parameter BLOCK rather than a topic, and so must be
# left exactly as written.
#
# nav2_params.yaml's costmaps say
#
#     observation_sources: scan
#     scan:
#       topic: /scan
#       raytrace_max_range: 8.0      ... and six more tuned settings
#
# The `scan` on the first line is not a topic - it is the NAME OF THE BLOCK on
# the second. Rewriting it to /r1/scan leaves the block still called `scan`, so
# the obstacle layer looks up a source it cannot find and FALLS BACK TO
# DEFAULTS for every one of those settings. It does not error: nav2_costmap_2d
# declares `<source>.topic` with the source name as its own default, so the
# topic accidentally comes out right and the layer runs - just with default
# ranges and default data_type instead of the tuned ones.
#
# The `topic: /scan` on the third line IS a topic and is rewritten normally.
...
# NO SPEED REWRITES HERE, DELIBERATELY. An earlier version of this file also
# raised desired_linear_vel to 0.85, the velocity smoother to 1.0 and the diff
# drive controller's own cap to 1.0, for routes 40 m long. Those are TUNING, not
# the frame/topic surgery this module exists to do, and they trade against the
# thing this fleet is being built on: wheel-odometry error grows with speed on a
# skid-steer, and the same run that raised them ended 1.44 m mislocalised. If a
# later step needs the speed, raise it there, deliberately, with the localisation
