...
once. Written out by hand for the fleet, the file would be about four thousand
lines of near-duplicate YAML with the namespace as the only difference, and the
next robot added to ROBOTS in aws_hospital_fleet.launch.py would silently not
...
# footprint, so four robots can be told apart at a glance in one view.
...
def _robot_model(ns):
    ...
    # (see the MoveIt comment in aws_hospital_fleet.launch.py), while the TF
...
def _robot_group(idx, ns, arm, nav):
    ...
    if arm:
        ...
        # bridge comment in aws_hospital_fleet.launch.py, where bridging four
...
def fleet_rviz_config(robots, arm_robots=(), nav_robots=(),
    ...
    `robots` is the fleet launch file's ROBOTS: (namespace, x, y, yaw) tuples.
    `arm_robots` and `nav_robots` are its ARM_ROBOTS and NAV_ROBOTS, which
